...
import com.stt.smspreparationmanager.entity.MaySms;
import com.stt.smspreparationmanager.repository.MaySmsRepository;
 ...
 * @author Enrique
 */
public class MaySmsHelper extends SmsHelper{
    
    private static final int monthInt = 4;
    private static final int daysThisMonth = 31;
    private static final String nameOfMonth = "Mayo";

    @Autowired
    private MaySmsRepository repo;

    public MaySmsHelper(MaySmsRepository repo, int amt) {
        this.repo = repo;
        this.amt = amt;
    }

    public void start(String systemId, String carrierCharCode, String messageType, int amt) {
        List<MaySms> batch = new ArrayList<>();
        for (int i = 0; i < amt; i++) {
            MaySms msg = new MaySms();
            ...
            msg.setMessagesText("Este es un mensaje en Mayo; " +cal.toString());
