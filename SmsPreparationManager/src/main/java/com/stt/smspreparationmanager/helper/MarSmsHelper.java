...
import com.stt.smspreparationmanager.entity.MarSms;
import com.stt.smspreparationmanager.repository.MarSmsRepository;
 ...
 * @author Enrique
 */
public class MarSmsHelper extends SmsHelper{
    
    private static final int monthInt = 2;
    private static final int daysThisMonth = 31;
    private static final String nameOfMonth = "Marzo";

    @Autowired
    private MarSmsRepository repo;

    public MarSmsHelper(MarSmsRepository repo, int amt) {
        this.repo = repo;
        this.amt = amt;
    }

    public void start(String systemId, String carrierCharCode, String messageType, int amt) {
        List<MarSms> batch = new ArrayList<>();
        for (int i = 0; i < amt; i++) {
            MarSms msg = new MarSms();
            ...
            msg.setMessagesText("Este es un mensaje en Marzo; " +cal.toString());
