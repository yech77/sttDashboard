...
import com.stt.smspreparationmanager.repository.NovSmsRepository;
 ...
 * @author Enrique
 */
public class NovSmsHelper extends SmsHelper{
    
    private static final int monthInt = 10;
    private static final int daysThisMonth = 30;
    private static final String nameOfMonth = "Noviembre";

    @Autowired
    private NovSmsRepository repo;

    public NovSmsHelper(NovSmsRepository repo, int amt) {
        this.repo = repo;
        this.amt = amt;
    }

    public void start(String systemId, String carrierCharCode, String messageType, int amt) {
        List<NovSms> batch = new ArrayList<>();
        for (int i = 0; i < amt; i++) {
            NovSms msg = new NovSms();
            ...
            msg.setMessagesText("Este es un mensaje en Noviembre; " +cal.toString());
