...
import com.stt.smspreparationmanager.entity.JunSms;
import com.stt.smspreparationmanager.repository.JunSmsRepository;
 ...
 * @author Enrique
 */
public class JunSmsHelper extends SmsHelper{
    
    private static final int monthInt = 5;
    private static final int daysThisMonth = 30;
    private static final String nameOfMonth = "Junio";

    @Autowired
    private JunSmsRepository repo;

    public JunSmsHelper(JunSmsRepository repo, int amt) {
        this.repo = repo;
        this.amt = amt;
    }

    public void start(String systemId, String carrierCharCode, String messageType, int amt) {
        List<JunSms> batch = new ArrayList<>();
        for (int i = 0; i < amt; i++) {
            JunSms msg = new JunSms();
            ...
            msg.setMessagesText("Este es un mensaje en Junio; " +cal.toString());
