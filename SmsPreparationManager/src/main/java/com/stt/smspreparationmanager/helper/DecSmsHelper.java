...
import com.stt.smspreparationmanager.entity.DecSms;
import com.stt.smspreparationmanager.repository.DecSmsRepository;
 ...
 * @author Enrique
 */
public class DecSmsHelper extends SmsHelper{
    
    private static final int monthInt = 11;
    private static final int daysThisMonth = 31;
    private static final String nameOfMonth = "Diciembre";

    @Autowired
    private DecSmsRepository repo;

    public DecSmsHelper(DecSmsRepository repo, int amt) {
        this.repo = repo;
        this.amt = amt;
    }

    public void start(String systemId, String carrierCharCode, String messageType, int amt) {
        List<DecSms> batch = new ArrayList<>();
        for (int i = 0; i < amt; i++) {
            DecSms msg = new DecSms();
            ...
            msg.setMessagesText("Este es un mensaje en Diciembre; " +cal.toString());
