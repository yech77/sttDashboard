...
import com.stt.smspreparationmanager.entity.JulSms;
import com.stt.smspreparationmanager.repository.JulSmsRepository;
 ...
 * @author Enrique
 */
public class JulSmsHelper extends SmsHelper{
    
    private static final int monthInt = 6;
    private static final int daysThisMonth = 31;
    private static final String nameOfMonth = "Julio";

    @Autowired
    private JulSmsRepository repo;

    public JulSmsHelper(JulSmsRepository repo, int amt) {
        this.repo = repo;
        this.amt = amt;
    }

    public void start(String systemId, String carrierCharCode, String messageType, int amt) {
        List<JulSms> batch = new ArrayList<>();
        for (int i = 0; i < amt; i++) {
            JulSms msg = new JulSms();
            ...
            msg.setMessagesText("Este es un mensaje en Julio; " +cal.toString());
