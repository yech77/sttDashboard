 ...
 * @author Enrique
 */
public class OctSmsHelper extends SmsHelper{
    
    private static final int monthInt = 9;
    private static final int daysThisMonth = 31;
    private static final String nameOfMonth = "Octubre";

    @Autowired
    private OctSmsRepository repo;

    public OctSmsHelper(OctSmsRepository repo, int amt) {
        this.repo = repo;
        this.amt = amt;
    }

    public void start(String systemId, String carrierCharCode, String messageType, int amt) {
        List<OctSms> batch = new ArrayList<>();
        for (int i = 0; i < amt; i++) {
            ...
            msg.setMessagesText("Este es un mensaje en Octubre; " +cal.toString());
    ...
    public void start(int amt) {
        ...
        System.out.println("Mensajes en Octubre: " + repo.count());
        System.out.println("Mensajes en Octubre: \n" + repo.findAll());
    }
}
