...
import com.stt.smspreparationmanager.entity.JanSms;
import com.stt.smspreparationmanager.repository.JanSmsRepository;
 ...
 * @author Enrique
 */
public class JanSmsHelper extends SmsHelper{
    
    private static final int monthInt = 0;
    private static final int daysThisMonth = 31;
    private static final String nameOfMonth = "Enero";

    @Autowired
    private JanSmsRepository repo;

    public JanSmsHelper(JanSmsRepository repo, int amt) {
        this.repo = repo;
        this.amt = amt;
    }

    public void start(String systemId, String carrierCharCode, String messageType, int amt) {
        List<JanSms> batch = new ArrayList<>();
        for (int i = 0; i < amt; i++) {
            JanSms msg = new JanSms();
            ...
            msg.setMessagesText("Este es un mensaje en Enero; " +cal.toString());
