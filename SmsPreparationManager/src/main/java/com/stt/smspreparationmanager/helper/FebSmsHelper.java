...
import com.stt.smspreparationmanager.entity.FebSms;
import com.stt.smspreparationmanager.repository.FebSmsRepository;
 ...
 * @author Enrique
 */
public class FebSmsHelper extends SmsHelper{
    
    private static final int monthInt = 1;
    private static final int daysThisMonth = 29;
    private static final String nameOfMonth = "Febrero";

    @Autowired
    private FebSmsRepository repo;

    public FebSmsHelper(FebSmsRepository repo, int amt) {
        this.repo = repo;
        this.amt = amt;
    }

    public void start(String systemId, String carrierCharCode, String messageType, int amt) {
        List<FebSms> batch = new ArrayList<>();
        for (int i = 0; i < amt; i++) {
            FebSms msg = new FebSms();
            ...
            msg.setMessagesText("Este es un mensaje en Febrero; " +cal.toString());
