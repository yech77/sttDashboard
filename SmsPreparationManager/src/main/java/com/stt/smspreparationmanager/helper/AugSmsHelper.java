...
import com.stt.smspreparationmanager.entity.AugSms;
import com.stt.smspreparationmanager.repository.AugSmsRepository;
 ...
 * @author Enrique
 */
public class AugSmsHelper extends SmsHelper{
    
    private static final int monthInt = 7;
    private static final int daysThisMonth = 31;
    private static final String nameOfMonth = "Agosto";

    @Autowired
    private AugSmsRepository repo;

    public AugSmsHelper(AugSmsRepository repo, int amt) {
        this.repo = repo;
        this.amt = amt;
    }

    public void start(String systemId, String carrierCharCode, String messageType, int amt) {
        List<AugSms> batch = new ArrayList<>();
        for (int i = 0; i < amt; i++) {
            AugSms msg = new AugSms();
            ...
            msg.setMessagesText("Este es un mensaje en Agosto; " +cal.toString());
