...
import com.stt.smspreparationmanager.entity.AprSms;
import com.stt.smspreparationmanager.repository.AprSmsRepository;
 ...
 * @author Enrique
 */
public class AprSmsHelper extends SmsHelper{
    
    private static final int monthInt = 3;
    private static final int daysThisMonth = 30;
    private static final String nameOfMonth = "Abril";
    ...
    @Autowired
    private AprSmsRepository repo;

    public AprSmsHelper(AprSmsRepository repo, int amt) {
        this.repo = repo;
        this.amt = amt;
    }

    public void start(String systemId, String carrierCharCode, String messageType, int amt) {
        List<AprSms> batch = new ArrayList<>();
        for (int i = 0; i < amt; i++) {
            AprSms msg = new AprSms();
            ...
            msg.setMessagesText("Este es un mensaje en Abril; " +cal.toString());
